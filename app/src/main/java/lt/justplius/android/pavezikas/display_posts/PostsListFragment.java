...
import lt.justplius.android.pavezikas.add_post.AddPostActivityTwoPane;
...
public class PostsListFragment extends ListFragment {
        ...
        switch (item.getItemId()) {
            case R.id.add:
                if (((PostsListActivity) getActivity()).isTwoPane()) {
                    Intent intent = new Intent(getActivity(), AddPostActivityTwoPane.class);
    ...
    // Receive event via Otto event bus
    @Subscribe
    public void downloadPosts(DownloadPostsEvent event) {
        // TODO update model
        downloadPosts(Filter.FILTER_NONE);
    }
